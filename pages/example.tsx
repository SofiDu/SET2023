...
import { Button, Title, keys } from "@mantine/core";

export default function Todo() {
  const { todos, addTodo, deleteTodo } = useTodo(["Todo #1", "Todo #2", "Todo 3"]);
        ...
        <div className="h-96 p-4 flex flex-col gap-2 overflow-y-scroll">
          {todos.map((todo, index) => (
            ...
            >
              ...
              <Button bg="red" onClick={() => deleteTodo(index)}>Delete</Button>
            </div>
          ))}
        </div>
      </div>
